...
class Trader:
    def run(self, state: TradingState) -> Dict[str, List[Order]]:
        ...
        for product in state.order_depths.keys():
            if product == "PEARLS" and product in state.position:
                ...
                if(state.position.get(product, 0) >= 0):
                    ...
                    if len(order_depth.sell_orders) > 0:
                        ...
                        if best_ask <= acceptable_buy_price and state.position.get(product, 0) >= -20 and state.position.get(product, 0) <= 20:
                    ...
                    if len(order_depth.buy_orders) > 0:
                        ...
                        # Also makes sure that our position is within range
                        if best_bid >= acceptable_sell_price and state.position.get(product, 0) >= -20 and state.position.get(product, 0) <= 20:
